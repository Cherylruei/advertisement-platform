...
function handleCardClicked({ target }) {
  ...
  if (node) {
    ...
    // inflencer.name
    console.log('idArr:', idArr[1])
    // influencer.id
    const num = Number(idArr[idArr.length - 1])
    // 若點選第二個 = idArr[1] = id: 2 = Number(2) = num 
    console.log("num:", num)
    cards[activeCard].classList.remove('active')
    cards[num - 1].classList.add('active')
    // 因為前面num 已經轉換成id的數字, 若要使用這個數字取到對應的位置，要使用 num-1
    activeCard = num - 1
    console.log("activeCard:", activeCard)
    // 若點選第二張卡片(id=2)，activeCard = num - 1, 也就是卡片中的第一個位置。在[0]之後
  }
}
